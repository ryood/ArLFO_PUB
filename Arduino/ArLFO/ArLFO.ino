...
#include "wavetable_12bit_4k_sine.h"
#define COUNT_OF_ENTRIES  (4096)

#define PIN_CHECK   (0)
#define UART_TRACE  (0)
#define TITLE_STR1  ("Arduino LFO")
#define TITLE_STR2  ("20250115")

#define MAX_RATE_H  (50.0f)
#define MAX_RATE_L  (5.0f)
#define RATE_RATIO_H  (1024.0f / MAX_RATE_H)
#define RATE_RATIO_L  (1024.0f / MAX_RATE_L)
...
// Pin Assign
const int PotRate = 1;          // A1
const int PotPulseWidth = 0;    // A0
...
void sync_in()
{
  int level = digitalRead(SyncInPin);
  if (level == HIGH) {
    phaccu = 0;
  }
  digitalWrite(Led4, level);
}
...
void setup()
{
  ...
  attachInterrupt(digitalPinToInterrupt(SyncInPin), sync_in, CHANGE);
...
void loop()
{
  ...
  // Speed
  bool speedHigh = digitalRead(SwSpeed);
  
  // DDS
  drate = (float)(analogRead(PotRate) + 1) / (speedHigh ? RATE_RATIO_H : RATE_RATIO_L);
  tword_m = pow(2, 32) * drate / refclk;  // calulate DDS new tuning word

  // Pulse Width
  pulse_width = (uint32_t)analogRead(PotPulseWidth) * COUNT_OF_ENTRIES / 1024;
  ...
  switch (waveshape_sel) {
    ...
    case WS_TRI:
    case WS_SQR:
      portd_bits = (1 << (waveshape_sel + 4));
      break;
    case WS_SAWUP:
      portd_bits = 0x30; // LED:xoo
      break;
    case WS_SAWDOWN:
      portd_bits = 0x60; // LED:oox
      break;
    case WS_SH:
      portd_bits = 0x70;  // LED:ooo
      break;
  }
  PORTD = portd_bits | (PORTD & 0x8f);
